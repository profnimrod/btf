"""Lab E: AWQ-style activation-aware 4-bit quantization (Ch. 14).
...
The GPU path for 8B-class HF models uses autoawq; see env/requirements.lock.
"""
...
def plan(a):
    print(json.dumps({
        "model": a.model, "calib": a.calib, "scheme": f"AWQ int{a.bits}",
...
def native(a):
    ...
    scales = collect_activation_scales(model, ids)
    n = quantize_model(model, bits=a.bits, group=a.group, method="rtn",
                       act_scales=scales)
    Path(a.out).parent.mkdir(parents=True, exist_ok=True)
    torch.save({"model": model.state_dict(), "cfg": model.cfg.__dict__,
                "quant": {"scheme": f"awq-int{a.bits}", "group": a.group,
                          "calib": a.calib}}, a.out)
    print(f"[awq] quantized {n} projections to int{a.bits} "
          f"(group {a.group}, activation-aware) -> {a.out}")
    print("[awq] now run: python eval/harness.py --ckpt %s  then eval/delta.py --gates"
          % a.out)
...
def main():
    ...
    ap.add_argument("--out", default="ckpt/awq-int4.pt")
