...
def mine(src: str, qa_history: str | None = None):
    ...
    if qa_history and Path(qa_history).exists():
        docs = {p.stem: p.read_text(encoding='utf-8') for p in Path(src).rglob("*.txt")}
        for line in Path(qa_history).read_text(encoding='utf-8').splitlines():
            if not line.strip():
                continue
            r = json.loads(line)
    ...
    for p in sorted(Path(src).rglob("*.txt")):
        text, doc = p.read_text(encoding='utf-8'), p.stem
        blocks = [b.strip() for b in re.split(r"\n\s*\n", text) if b.strip()]
        if not blocks:
            continue
        title = blocks[0].split("\n")[0]
